...
if( strlen($rows3->result[0]['device_mac_address']) ){
	...
	if( ! strlen($rows3->result[1]['device_address']) ){
		//user has one and only one device, go for it directly
		$wanted_device = $rows3->result[0]['device_address'] ;
	}
}
...
if(! strlen($wanted_device)) {
	...
	while( strlen($rows3->result[$conta1]['device_address']) ){
		echo "<option value=\"" . $rows3->result[$conta1]['device_address'] . "\"> EXT=" . $rows3->result[$conta1]['extension'] .  " LABEL=" . $rows3->result[$conta1]['display_name'] .  " INT_NAME=" . $rows3->result[$conta1]['effective_caller_id_name'] .  " OUT_NUM=" . $rows3->result[$conta1]['outbound_caller_id_number'] . " MODEL=" . $rows3->result[$conta1]['device_template'] .  " MAC=" . $rows3->result[$conta1]['device_address'] .     "   </option> \n";
		$conta1++;
	}
	echo "                                    </select> \n";
	echo "                                    <br/> \n";
